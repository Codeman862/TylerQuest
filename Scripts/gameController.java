...
public class gameController {
	...
	public static int findListDistance(ArrayList list, ArrayList targetList,
			int unitIndex) {
		...
		for (int a = 0; a < targetlistsize; a++) {
			...
			if (targetSol.isDead == true
					|| targetSol.invisible_to_player == true) {
			} else {
	...
	public gameController() {
		...
		waveTimer[0] = 1000; // timers are either in milliseconds or seconds, I
								// forget which
		waveTimer[1] = 1000;
		waveTimer[2] = 1000;
	}
	...
	public long[] resetTimer(long[] timer, float duration, float percentError) {
		...
		// long difference=(long) ((1-percentError)*timer[2]); //uses base
		// duration to set actual duration

		// long timerRandomness=(long)
		// (Math.random()*(difference*2)+(timer[2]-difference));//gabe check
		// math
